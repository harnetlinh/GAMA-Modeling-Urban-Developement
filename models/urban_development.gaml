...
//	}
	...
	list<cell> list_overlapped_greenspace <- cell overlapping shape;
	
	reflex check_overlap_greenspace {
		loop i over: list_overlapped_greenspace {
			i.available <- false;
		}
	}
	reflex check_pollution_greenspace {
		list<cell> _current_cell <- (cell overlapping self);
//		write _current_cell;
		cell current_cell <- _current_cell[0];
		list<cell> neighbors_road <- current_cell.neighbors;
		loop i over: neighbors_road {
			float nb_neighbor_greenspace <- 0.0;
			list<cell> neighbors <- i.neighbors;	
			loop j over: neighbors{
				list<agent> sp <- agents_inside(j);
				if(!empty(sp)){
					if(contains(sp[0].name,"greenspace")){
						nb_neighbor_greenspace <- nb_neighbor_greenspace + 1.0;
					}
				}
			}
			loop j over: neighbors {
				j.pollution <- (nb_neighbor_greenspace);
			}
		}
	}
}
